(ns truebill.main
  "Kotodama WASM entrypoint for the truebill clean-room actor (L4) — Clojure port.
  ...
  facts (`truebill.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "truebill")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"truebill.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "truebill-compat" :tier tier :entities entities} 200])
...
(defn start! [] :truebill-compat/ready)
